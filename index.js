...
// ========== SESSION_ID (ENV/CONFIG-VAR) PERSISTENCE ==========
// A SESSION_ID string (e.g. from a platform Config Var/secret) is the one
// backup that survives even when there is no database AND local disk is
// wiped on every restart. Previously we only ever *generated* a SESSION_ID
// after pairing — nothing ever read it back in. That meant setting it as an
// env var did nothing on boot, which defeated the whole point of offering
// it as a durability option. This decodes it back into a real creds.json
// so it can seed the session directory exactly like the local/db backups do.
const SESSION_ID_PREFIX = 'Ultra-X:~';

function decodeSessionId(sid) {
    if (!sid || typeof sid !== 'string') return null;
    const raw = sid.trim();
    if (!raw.startsWith(SESSION_ID_PREFIX)) return null;
    try {
        const encoded = raw.slice(SESSION_ID_PREFIX.length);
        const buf = Buffer.from(encoded, 'base64');
        // Sanity check — make sure it actually decodes to JSON creds, not garbage.
        JSON.parse(buf.toString('utf8'));
        return buf;
    } catch (e) {
        console.error('[ SESSION ] SESSION_ID failed to decode:', e.message);
        return null;
    }
}

async function restoreSessionFromEnv(destDir) {
    const credsBuf = decodeSessionId(process.env.SESSION_ID);
    if (!credsBuf) return false;
    try {
        fs.mkdirSync(destDir, { recursive: true });
        fs.writeFileSync(path.join(destDir, 'creds.json'), credsBuf);
        console.log('[ SESSION ] Restored creds.json from SESSION_ID env var.');
        return true;
    } catch (e) {
        console.error('[ SESSION ] Restore from SESSION_ID failed:', e.message);
        return false;
    }
}
...
async function restoreSessionBackup(destDir) {
    ...
    // Still nothing locally and no database hit — last resort is a SESSION_ID
    // env var (Option 2 login), which survives even with zero DB configured.
    if (!localHasFiles) {
        const restored = await restoreSessionFromEnv(SESSION_BACKUP_DIR);
        localHasFiles = restored && fs.readdirSync(SESSION_BACKUP_DIR).length > 0;
    }
...
async function spawnBot(attempt = 1) {
    ...
    // Only restore from backup if the bot's session dir is actually missing/empty
    // (it cleared its own session, or the whole filesystem was wiped by a platform
    // restart — restoreSessionBackup falls back to the database / SESSION_ID env
    // in that case).
    //
    // Previously this restored unconditionally on every spawn, which meant a plain
    // in-process restart (e.g. the .restart/.update command, or any ordinary crash)
    // would blow away the bot's current, perfectly valid session with the stale
    // snapshot taken back at initial pairing — forcing a re-pair even though nothing
    // on disk was actually lost. If the session is still there, just refresh the
    // backup from it so it stays current for the case that DOES need it later.
    const sessionDir = path.join(botRepoPath, 'session');
    const liveSessionExists = fs.existsSync(sessionDir) && fs.readdirSync(sessionDir).length > 0;

    if (liveSessionExists) {
        try {
            await saveSessionBackup(sessionDir);
        } catch (e) {
            console.error('[ BOT ] Failed to refresh session backup:', e.message);
        }
    } else {
...
// — restoreSessionFromEnv() rebuilds creds.json straight from it on boot.
function generateSessionId() {
    try {
        const credsPath = path.join(AUTH_DIR, 'creds.json');
        if (!fs.existsSync(credsPath)) return null;
        const encoded = Buffer.from(fs.readFileSync(credsPath)).toString('base64');
        return `${SESSION_ID_PREFIX}${encoded}`;
    } catch (e) {
...
app.post('/api/session', async (req, res) => {
    ...
    // Validate it actually decodes before accepting it, so a bad paste fails
    // fast with a clear message instead of silently breaking the next restart.
    const credsBuf = decodeSessionId(sid);
    if (!credsBuf) {
        return res.json({ success: false, error: 'Session ID is malformed or unreadable. Copy it exactly as shown.' });
    }
    ...
    // Seed auth_info_pair + the persistent backup + the DB (if configured) right
    // away, so the very next restart already has it in every tier, not just env.
    try {
        fs.mkdirSync(AUTH_DIR, { recursive: true });
        fs.writeFileSync(path.join(AUTH_DIR, 'creds.json'), credsBuf);
        await saveSessionBackup(AUTH_DIR);
    } catch (e) {
        console.error('[ SESSION ] Failed to seed auth dir from pasted Session ID:', e.message);
    }
      ...
      if (s.dbConfigured) {
        ...
        sessionBlock = \`
          <div class="code-steps" style="text-align:left;">
            ...
            the bot picks it up automatically on the next boot to restore this exact session safely.
...
(async () => {
  try {
    ...
    if (sessionAvailable) {
        ...
        // itself just got rehydrated (from local backup, database, or SESSION_ID
        // env var) above. Reflect that on the dashboard instead of showing the
        // pairing form again.
        if (pairState.status !== 'connected') {
